...
const int MAX_PAYLOAD_LEN = 10*1024*1024;
...
bool CHttpDataPacket::RecombinePacket(void)
{
    ...
    if (strHeader.find("application/x-rtsp-tunnelled") != std::string ::npos)
    {
        m_nPayLoadLen = 0xFFFFFFF;
    }

	// 获取已经收到的数据长度
	//int nFist = strHeader.find(HTTP_DEFINE::DOUBLE_ENTER_LINE) + strlen(HTTP_DEFINE::DOUBLE_ENTER_LINE);
	tuint32 nRevLen = m_RevBuff.m_nLen - nHeaderLen;//strHeader.length() - nFist;
	if (m_nPayLoadLen > MAX_PAYLOAD_LEN)
	{
		P_DATA_PACK pPack = new DATA_PACK;
	...
	else if (m_nPayLoadLen > nRevLen)		// 如果数据还没有接收完成
	{
		return false;
	}
