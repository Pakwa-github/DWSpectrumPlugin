...
namespace HTTP_DEFINE
{
    ...
    static const char AUINFO[] = "auInfo=";
	static const int HTTP_HEADER_LEN = 1024;
    static const char SECURITY_HEADERS[] = "X-XSS-Protection: 1; mode=block\r\nX-Content-Type-Options: nosniff\r\nX-Frame-Options:SAMEORIGIN\r\nSet-Cookie:Secure; HttpOnly";
/*http  请求方法*/
    enum HTTP_FUNCTION {
        HTTP_UNKNOW = 0,
        HTTP_GET,
	...
	{
		_HTTP_PROTOCOL_(): m_nDataLen(0), m_nPayLoadLen(0),  m_pBuff(0){}
		~_HTTP_PROTOCOL_()
		{
			if (m_pBuff != 0)
			{
		...
		_HTTP_PROTOCOL_ &operator=(const _HTTP_PROTOCOL_ &other)
		{
			if (this != &other)
			{
                ...
                this->m_strURLParam = other.m_strURLParam;
		...
		std::string m_strURLParam;                          // URL参数字符串
	...
	static const std::map<string, string>::value_type ContentType[] =
	{
        ...
        std::map<string, string>::value_type(".js", "application/x-javascript"),
        std::map<string, string>::value_type(".exe", "application/octet-stream"),
        std::map<string, string>::value_type(".pkg", "application/octet-stream"),
        std::map<string, string>::value_type(".wasm", "application/wasm"),
        std::map<string, string>::value_type("", "application/octet-stream"),
	};

    static const std::map<string, string> g_ContentTypeMap(ContentType, ContentType+19);
	//////////////////////////////////////////////////////////////////////////
}
